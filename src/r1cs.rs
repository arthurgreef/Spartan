...
pub struct R1CSGens<G: Group> {
  pub gens_W: CommitGens<G>, // TODO: avoid pub(crate)
...
impl<G: Group> R1CSShape<G> {
  ...
  pub fn new(
    ...
    if num_io % 2 != 0 {
      return Err(R1CSError::OddInputLength);
    }
  ...
  fn multiply_vec(
    &self,
    z: &[G::Scalar],
  ) -> Result<(Vec<G::Scalar>, Vec<G::Scalar>, Vec<G::Scalar>), R1CSError> {
  ...
  pub fn is_sat(
    ...
    let res_eq: bool = {
      ...
      let res: usize = (0..self.num_cons)
        .map(|i| if Az[i] * Bz[i] == Cz[i] { 0 } else {
        dbg!(i);
        1 })
        .sum();
...
impl<G: Group> R1CSWitness<G> {
  ...
  pub fn new(S: &R1CSShape<G>, W: &[G::Scalar]) -> Result<R1CSWitness<G>, R1CSError> {
    if S.num_vars != W.len() {
...
impl<G: Group> R1CSInstance<G> {
  ...
  pub fn new(
    ...
    comm_W: &Commitment<G>,
    X: &[G::Scalar],
  ) -> Result<R1CSInstance<G>, R1CSError> {
    if S.num_io != X.len() {
      Err(R1CSError::InvalidInputLength)
    } else {
      Ok(R1CSInstance {
        comm_W: *comm_W,
        X: X.to_owned(),
...
impl<G: Group> RelaxedR1CSWitness<G> {
  ...
  pub fn fold(
    ...
    T: &[G::Scalar],
    r: &G::Scalar,
  ) -> Result<RelaxedR1CSWitness<G>, R1CSError> {
...
impl<G: Group> RelaxedR1CSInstance<G> {
  ...
  pub fn fold(
    ...
    r: &G::Scalar,
  ) -> Result<RelaxedR1CSInstance<G>, R1CSError> {
    ...
    if self.counter > 0 {
      if self.Y_last.len() != U2.X.len() / 2 {
        return Err(R1CSError::InvalidInputLength);
      }
      for i in 0..self.Y_last.len() {
        if self.Y_last[i] != U2.X[i] {
          return Err(R1CSError::InputOutputMismatch);
        }
      }
    }
