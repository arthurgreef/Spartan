...
#[test]
fn test_mult_mod() {
  type G = zerocaf::edwards::EdwardsPoint;
  ...
  let q_val = BigInt::from_str_radix(
    "54743953705193878336058555664764906530834440901767868145822957",
    10,
  )
  .unwrap();
  let r_val = BigInt::from_str_radix(
    "47807904131255048459016820388070496464748585607776212153724633362673718867155927782035675",
    10,
  )
  .unwrap();
  ...
  let _ = synthesize_mult_mod(&mut cs, &a_val, &b_val, &m_val, &q_val, &r_val,64, 6);
